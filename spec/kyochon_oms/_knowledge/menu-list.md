---
title: kyochon_oms 메뉴코드 목록
description: kyochon_oms 전체 메뉴코드·메뉴명·상위그룹 레지스트리. 메뉴 단위 스킬(KB_100/KB_200 등)이 메뉴 정보를 조회할 때 사용.
status: draft
version: 0.1.0
repo_role: ai-hub
...
**설계** — `spec/kyochon_oms/{메뉴코드}/` 설계 문서
...
**프로토** — `prototype/kyochon_oms/{메뉴코드}/` 검증 화면 (모바일은 `{메뉴코드}m`)
