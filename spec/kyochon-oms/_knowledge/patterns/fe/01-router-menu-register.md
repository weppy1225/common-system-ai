...
> 전제: Vue3+Vite+Pinia, 한 코드베이스 → **두 UI 시스템** 쇼핑몰(가맹점 `/bc`, e-commerce 스타일·AUIGrid 미사용)·관리자(Admin `/be`, WMS식 AUIGrid)·모바일(`/bm`) 모드분기. 두 시스템 차이 → `spec/kyochon-oms/_knowledge/install-guide/01-startup-guide.md` §3.0.
