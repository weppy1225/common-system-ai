...
description: spec/{메뉴}/ 설계 문서의 접미사별 의미·생성 스킬 규약. 실제 전체 파일 인벤토리는 20-md-index.html(파일 지도) 참조.
status: active
version: 4.0.0
wms_meta: true
...
`spec/{메뉴}/` 한 메뉴의 설계 문서는 아래 접미사 체계를 따른다. **번호 순서 = 읽는 순서**(왜→무엇→설계→흐름→미결).
...
재생성: `python scripts/gen-md-map.py`. 특정 메뉴 파일은 `spec/{메뉴}/` 직접 확인.
