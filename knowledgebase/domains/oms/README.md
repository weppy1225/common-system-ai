...
숨은 전제: OMS = 전통 Spring(Boot 아님) · Ant WAR 빌드 · OMS=PostgreSQL + ERP=SQL Server 멀티 DB · Vue3+Vite+Pinia. FE는 **한 레포 두 UI 시스템** — 쇼핑몰(가맹점 `/bc`, e-commerce 스타일·AUIGrid 미사용)과 관리자(Admin `/be`, WMS식 AUIGrid). 상세 → `install-guide/oms-01-startup-guide.md` §3.0.

## 무엇을 넣나 (프로젝트가 달라도 같은 것)

| 폴더/파일 | 내용 |
|---|---|
| `install-guide/oms-01-startup-guide.md` | OMS 고유 기동·빌드·배포 차이 (Ant WAR, 멀티 DB, Vite mode 분기) |
| `patterns/be/` | BE 레이어 작성 패턴(`oms-01-layer-pattern`), 채번 공통모듈(`oms-03-numbering-module`) |
| `patterns/db/` | SQL 쿼리 스타일(`oms-01-sql-query-style`), MyBatis 컨벤션(`oms-02-mybatis-convention`), 네이밍 규칙(`oms-03-naming-rule`) |
| `patterns/fe/` | 라우터·메뉴 등록(`oms-01-router-menu-register`), 공통코드 commCdStore(`oms-02-common-code-commCdStore`) |
