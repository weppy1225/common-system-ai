...
> 공통 네이밍 골격(`lower_snake_case` 일반규칙·표준 약어 seq/no/cd/nm/ymd/hms/dt/qty/yn/sts·컬럼 유형별 패턴·감사컬럼 `reg_id`/`reg_dt`/`mod_id`/`mod_dt`·`if_*`/`fr_*`/`to_*`·생성 템플릿)은 [common 문서](../../20-database/20-rule/01-naming-rule.md)와 동일하다. 이 문서는 **OMS 고유 차이분만** 담는다.
...
- 컬럼 → camelCase 매핑(Java 필드 접미어)은 → `oms-ai/04-도메인-코드값.md §10`.
