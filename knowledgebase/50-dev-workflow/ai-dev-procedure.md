...
| 산출물 | 위치 | 생성 스킬 |
|---|---|---|
| 화면요건 (`*-02-ui.md`) | `spec/{프로젝트}/{메뉴코드}/` | `/SD_310_UI` |
| DB 설계 (`db.md`) | `spec/{프로젝트}/{메뉴코드}/` | `/SD_db` |
| DB 반영 완료 (test/dev) | PostgreSQL | `/SD_db_apply` |
| API 명세 (`api.md`) | `spec/{프로젝트}/{메뉴코드}/` | `/SD_api` |
...
```powershell
# Windows (PowerShell) — {프로젝트}-be 디렉토리에서 실행
cd "$BE_DIR"   # 형제 BE 레포 (→ .claude/rules/repo-paths.md)
...
| 산출물 | 위치 |
|---|---|
| BE spec.md (API 명세 + VO/DTO) | `spec/{프로젝트}/{메뉴코드}/{메뉴코드}-05-api.md` |
| 화면요건 ui.md | `spec/{프로젝트}/{메뉴코드}/{메뉴코드}-02-ui.md` |
| 프로토타입 wireframe.html | `spec/{프로젝트}/{메뉴코드}/{메뉴코드}-02-wireframe.html` |
...
```powershell
# {프로젝트}-fe 디렉토리에서 실행
cd "$FE_DIR"   # 형제 FE 레포 (→ .claude/rules/repo-paths.md)
