...
PostgreSQL DB에서 테이블 정의와 FK 관계를 직접 읽어  
`output\03 설계(SD)\SD.211-ERD_{업체명}_{YYMMDD}.html` ERD 뷰어 파일을 생성(또는 갱신)한다.

기존 `SD.211-ERD_*.html` 파일을 템플릿으로 사용하고  
`const TABLES=[...]`·`const FKS=[...]` 두 섹션만 DB 최신 상태로 교체한다.  
뷰어 코드(CSS·JS 함수·SVG 마커 등)는 템플릿에서 그대로 유지한다.
...
| ERD 템플릿 | `output\03 설계(SD)\SD.211-ERD_*.html` (최신 기존 파일) |
| 출력 | `output\03 설계(SD)\SD.211-ERD_{업체명}_{YYMMDD}.html` |
...
$outputFile  = "$outputDir\SD.211-ERD_$($ARGUMENTS)_${yymmdd}.html"
...
if (-not $templateFile) {
    Write-Error "ERD 템플릿 파일 없음. output\03 설계(SD)\SD.211-ERD_*.html 이 필요합니다."
    return
}
...
$html = [regex]::Replace($html, '<title>ERD - [^<]+</title>',
    "<title>ERD - $($ARGUMENTS) WMS ($yymmdd)</title>")

# sidebar 헤더 교체
$html = [regex]::Replace($html, 'ERD Viewer · [^<]+',
    "ERD Viewer · $($ARGUMENTS) WMS")
```
