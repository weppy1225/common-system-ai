...
### Step 0-1 — DB 스키마 문서 참조 (BLOCKING — psql 실행 전)

MUST: 테이블명·컬럼명을 사용하기 전에 **반드시** 아래 스키마 문서를 먼저 읽는다.
이유: 탐색 쿼리(`LIKE '%...%'`) 없이 정확한 테이블명·컬럼명을 사용해 불필요한 왕복 쿼리를 막는다.

경로 기준: `$AI_DIR/spec/kyochon_oms/_knowledge/db-schema/`

| 조회 도메인 | 참조 문서 |
|---|---|
| 전체 테이블 목록·그룹 | `spec/kyochon_oms/_knowledge/db-schema/00-tables-overview.md` |
...
| 공통코드 값 (`sm_comm_h`/`sm_comm_d`) | `spec/kyochon_oms/_knowledge/db-schema/90-common-code.md` |

**우선 적용 순서 (충돌 시):**
1. `spec/kyochon_oms/_knowledge/db-schema/` 문서 → 테이블명·컬럼명 1차 확인
2. 문서에 없는 경우 → 실 DB `pg_class`/`pg_attribute` comment 조회(§4-1·§4-2) 로 보완
...
### 4-2. comment 로 테이블 찾기 (fallback — 스키마 문서에 없을 때만)

MUST: 테이블을 찾을 때 **먼저 §Step 0-1 스키마 문서**를 확인한다. 문서에 없는 경우에만 아래 탐색 쿼리를 실행한다.
...
| 강도 | 규칙 |
| ---- | ---- |
| MUST | psql 실행 전 **Step 0-1 스키마 문서**(`spec/kyochon_oms/_knowledge/db-schema/`)를 먼저 읽는다 |
| NEVER | 스키마 문서 확인 없이 테이블명·컬럼명을 이름만 보고 추정한다 |
...
| MUST | 스키마 문서에 없는 컬럼은 `\d <테이블>` 또는 comment 조회로 실 DB에서 확인 |
