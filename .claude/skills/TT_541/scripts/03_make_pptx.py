...
입력:
  deliverables/30-output/05 이행(TT)/tmp_541/capture_config.json
  deliverables/30-output/05 이행(TT)/tmp_541/screens/{메뉴코드}/*.png
  deliverables/30-output/05 이행(TT)/tmp_541/screens/{메뉴코드}/coords.json

템플릿:
  deliverables/10-templates/05 이행(TT)/TT.411-사용자매뉴얼(PC).pptx
  - 13.33 × 7.5 인치 (16:9)
  - 제목바 #2D4B73 / 이미지영역 0~10in / 설명패널 10~13.33in / 페이지번호 우하단
  - 색상상수: 본 스크립트 상단의 COLOR_* 와 동일

출력:
  deliverables/30-output/05 이행(TT)/TT_541_사용자매뉴얼_{고객사명}.pptx
...
# ── 경로 (스크립트 위치 기준 동적 계산) ────────────────────────
# 본 스크립트는 .claude/skills/TT_541/scripts/ 안에 있다.
# parents[0]=scripts  parents[1]=TT_541  parents[2]=skills  parents[3]=.claude  parents[4]=repo root
SCRIPT_PATH = Path(__file__).resolve()
REPO_ROOT = SCRIPT_PATH.parents[4]
TEMPLATE = REPO_ROOT / 'deliverables' / '10-templates' / '05 이행(TT)' / 'TT.411-사용자매뉴얼(PC).pptx'
OUTPUT_DIR = REPO_ROOT / 'deliverables' / '30-output' / '05 이행(TT)'
TMP_DIR = OUTPUT_DIR / 'tmp_541'
...
OUT_FILE = OUTPUT_DIR / f"TT_541_사용자매뉴얼_PC_{CUSTOMER}.pptx"
