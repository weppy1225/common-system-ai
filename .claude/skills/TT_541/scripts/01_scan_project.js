#!/usr/bin/env node
/**
 * [TT_541] 1단계 — FE 프로젝트 스캔으로 메뉴 후보 추출
 *
 * 사용법:
 *   node 01_scan_project.js "<FE 프로젝트 경로>"
 *
 * 출력:
 *   output/05 이행(TT)/tmp/menu_candidates.json
 *
 * 동작:
...
// 윈도우 경로 → WSL 경로 자동 변환 (예: C:\foo\bar → /mnt/c/foo/bar)
if (/^[a-zA-Z]:\\/.test(fePath)) {
    const drive = fePath[0].toLowerCase();
    fePath = `/mnt/${drive}/` + fePath.slice(3).replace(/\\/g, '/');
}
...
// ── 4) 뷰포트 힌트 (PDA 파일명 = *sc01m, *sc01p 등) ───────────
function viewportHint(code) {
    if (/sc\d+m?$/i.test(code) || /^pda/i.test(code) || /^br[a-z]+\d+/i.test(code)) return 'pda';
    return 'desktop';
}
...
const menus = routes
    ...
    .map(r => ({
        ...
        path: r.path,
        viewportHint: viewportHint(r.code),
    }))
