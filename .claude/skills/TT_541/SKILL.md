...
> **템플릿(BLOCKING)**
> PPTX 생성에는 반드시 `deliverables/10-templates/05 이행(TT)/TT.411-사용자매뉴얼(PC).pptx` 를 열어 base 로 사용한다.
...
TEMPLATE  = deliverables/10-templates/05 이행(TT)/TT.411-사용자매뉴얼(PC).pptx
...
이 스킬은 FE 레포에 등록된 router 설정에서 **PC(데스크탑) 메뉴만** 추출한다. 아래 기준 중 하나라도 해당하면 **PDA로 분류하여 제외**한다.
...
3. `spec/{프로젝트}/{메뉴코드}/{메뉴코드}-02-ui.md` 또는 `menu-index.md` 에서 메뉴명 보완
...
**입력**:
- 템플릿: `deliverables/10-templates/05 이행(TT)/TT.411-사용자매뉴얼(PC).pptx` (필수)
...
- `spec/{프로젝트}/{메뉴코드}/{메뉴코드}-02-ui.md` 가 있으면 거기서 우선 참조 (메뉴명, 검색 조건, 그리드 컬럼, 업무규칙).
...
- **경로 공백·한글 처리**: `"deliverables\30-output\05 이행(TT)"` 처럼 공백·한글 포함 경로는 이스케이프 없이 따옴표로 감싼다.
