---
name: TT_542_WIN
description: 【PDA 사용자매뉴얼 PPTX 생성 (Windows 전용)】 Windows 네이티브(PowerShell) 환경에서 사용자가 지정한 프론트엔드 프로젝트의 실제 dev/배포 서버에 Playwright(헤드리스, 모바일 390×844)로 접속하여 PDA(모바일) 사용자 메뉴별 화면을 캡처하고, template/05 이행(TT)/사용자_매뉴얼_템플릿.pptx 를 base로 python-pptx 기반의 PDA 사용자매뉴얼 PPTX를 자동 생성합니다. PC(데스크탑) 메뉴는 자동 제외되며 별도 스킬 /TT_541_WIN 에서 처리합니다. PDA 자동 식별 기준: 라우트 경로에 `/bm/`·`/pda/`·`/mobile/` 포함, 부모 segment 가 `*m` 패턴(`iv3000m`·`md8000m` 등), 메뉴코드 끝이 `m`(`ivad01m`·`ivmvrq01m` 등). cloud-wms-doc 의 `dist-mobile/{그룹}/{메뉴}.html` 도 보조 메뉴명 매핑에 활용합니다. 라벨·테두리·배지·커넥터·설명패널은 모두 PPT 안의 도형(add_shape)으로 그려 PowerPoint 내부에서 직접 편집할 수 있도록 합니다. /TT_542_WIN 형식으로 실행하며 FE 프로젝트 경로·고객사명·BASE_URL·메뉴 목록·로그인 정보는 실행 시 묻습니다. 산출물은 output\05 이행(TT)\TT_542_사용자매뉴얼_PDA_{고객사명}.pptx 단일 파일로 떨어집니다. PDA 사용자 매뉴얼 작성, 모바일 사용자용 매뉴얼, PDA 화면 캡처 PPT, WMS 모바일 사용자 매뉴얼 PPTX 만들기 요청 시 반드시 이 스킬을 사용합니다. 사용자가 "PDA 사용자매뉴얼 만들어줘", "모바일 매뉴얼 PPT 뽑아줘", "TT_542_WIN 실행해줘", "PDA 화면 캡쳐해서 PPT 만들어줘", "모바일 사용자 매뉴얼 산출물 만들어줘" 라고 말해도 이 스킬을 사용합니다. 단, PC 사용자 매뉴얼이 필요한 경우는 /TT_541_WIN, 운영자 매뉴얼은 /TT_543_WIN 을 사용합니다.
type: skill
allowed-tools: Bash, PowerShell, Read, Write, Edit, Glob, Grep, AskUserQuestion
---

# PDA 사용자 매뉴얼 PPTX 자동 생성 스킬 (Windows 전용) [TT_542_WIN]
...
## 실행 스크립트
1. `.claude/skills/TT_542_WIN/scripts/01_scan_project.js` (Node.js) — FE 스캔 + PC 자동 제외 (PDA만 keep)
2. `.claude/skills/TT_542_WIN/scripts/02_capture_screens.js` (Node.js + Playwright chromium 헤드리스, 모바일 컨텍스트) — 메뉴별 캡처
3. `.claude/skills/TT_542_WIN/scripts/03_make_pptx.py` (Python + python-pptx + Pillow) — PPTX 생성
...
> **PC(데스크탑) 메뉴는 본 스킬의 범위가 아니다.** PC 메뉴는 `/TT_541_WIN` 스킬에서 별도로 처리한다. 본 스킬은 1단계에서 PC 라우트(`/be/...`)와 PC 메뉴 코드를 자동 제외하고 PDA 메뉴만 남긴다.

> **운영자 매뉴얼은 본 스킬의 범위가 아니다.** 운영자/관리자 매뉴얼은 `/TT_543_WIN` 스킬에서 처리한다.
...
| 입력 | 설명 | 예시 |
|---|---|---|
| **FE 프로젝트 경로** | dev 서버를 띄울 프론트엔드 소스 루트. router 자동 스캔에 사용 | `C:\zinide\workspace\cloud-wms-fe` |
...
> **TMP 디렉토리 분리:** TT_541_WIN 은 `tmp_541`, TT_542_WIN 은 `tmp_542`, TT_543_WIN 은 `tmp_543` 를 사용하여 동시에 실행해도 충돌하지 않게 한다.
...
본 스킬은 cloud-wms-fe 등 일반적인 router 구조에서 **PDA(모바일) 메뉴만** 추출한다. 아래 기준 중 하나라도 만족하면 **PDA로 간주하여 포함**하고, 아니면 PC로 간주하여 `rejected[]` 에 분리한다.
...
node ".claude\skills\TT_542_WIN\scripts\01_scan_project.js" "{FE경로}"
```
...
```json
{
  "fePath": "C:\\zinide\\workspace\\cloud-wms-fe",
  "framework": "vue3-vite",
  "devPort": 5173,
  ...
  "rejected": [
    { "code": "mdpr01", "name": "사은품관리", "reason": "PC 메뉴(/be/... 또는 코드 끝이 m이 아님) — /TT_541_WIN 에서 처리" },
    { "code": "iwrq01", "name": "입고예정", "reason": "PC 메뉴(/be/... 또는 코드 끝이 m이 아님) — /TT_541_WIN 에서 처리" }
  ],
...
cloud-wms-fe `views/bm/**/*.vue` 컴포넌트 기준.
...
### 4단계 — PPTX 생성 (모바일 종횡비, TT_541_WIN 양식 그대로)
...
> PPT 양식은 TT_541_WIN 과 동일하다. 차이점은 이미지 종횡비가 세로형(모바일)이라는 점뿐이며, `Geom` 클래스가 `min(IMG_COL_W/PX_W, IMG_AREA_H/PX_H)` 으로 자동 보존한다.

#### 색상 매핑 (TT_541_WIN 과 동일)
...
```
✓ PDA 사용자매뉴얼 PPTX 생성 완료 [TT_542_WIN]
...
자동 제외된 PC 메뉴 ({P}개) — /TT_541_WIN 에서 처리:
