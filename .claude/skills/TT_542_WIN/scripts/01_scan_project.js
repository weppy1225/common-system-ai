#!/usr/bin/env node
/**
 * [TT_542_WIN] 1단계 — FE 프로젝트 스캔으로 PDA(모바일) 사용자 메뉴 후보 추출
 *
 * 사용법:
...
// 본 스크립트는 .claude/skills/TT_542_WIN/scripts/ 안에 있으므로
...
// PDA 메뉴만 keep (PC 메뉴는 /TT_541_WIN 범위로 분리)
const menus = [];
const rejected = [];
for (const m of rawMenus) {
    if (isPdaMenu(m.code, m.path)) {
        ...
        rejected.push({ code: m.code, name: m.name, reason: 'PC 메뉴(/be/... 또는 코드 끝이 m이 아님) — /TT_541_WIN 에서 처리' });
    }
}
