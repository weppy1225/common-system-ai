---
name: TT_543_WIN
description: 【운영자매뉴얼 PPTX 생성 (Windows 전용)】 Windows 네이티브(PowerShell) 환경에서 사용자가 지정한 프론트엔드 + 백엔드 디렉토리를 자동 스캔하여 "운영자(관리자)가 시스템·사용자·권한·메뉴·공통코드·사업장·센터·창고 등을 설정하는 관리성 메뉴"만 자동 식별합니다. 식별된 메뉴들을 실제 dev/배포 서버에 Playwright(헤드리스, 한국어 로케일 ko-KR)로 접속하여 화면 캡처한 뒤, `template/05 이행(TT)/사용자_매뉴얼_템플릿.pptx`를 base로 python-pptx 기반의 운영자매뉴얼 PPTX를 자동 생성합니다. PPT 양식·레이아웃·색상·도형 라벨링은 모두 TT_541_WIN(PC 사용자매뉴얼) 스킬과 동일한 양식을 따르며, 차이점은 (1) 대상 메뉴를 운영자/관리자 메뉴로만 필터링, (2) 표지 제목이 "운영자 매뉴얼", (3) Windows 경로 사용입니다. 라벨·테두리·배지·커넥터·설명패널은 모두 PPT 안의 도형(add_shape)으로 그려 PowerPoint 내부에서 직접 편집할 수 있도록 합니다. /TT_543_WIN 형식으로 실행하며 FE 경로·BE 경로·고객사명·BASE_URL·로그인 정보는 실행 시 묻습니다. 산출물은 `output\05 이행(TT)\TT_543_운영자매뉴얼_{고객사명}.pptx` 단일 파일로 떨어집니다. 운영자 매뉴얼 작성, 관리자 매뉴얼 작성, 시스템 설정 매뉴얼, 운영자용 PPT 만들기 요청 시 반드시 이 스킬을 사용합니다. 사용자가 "운영자매뉴얼 만들어줘", "관리자 매뉴얼 PPT 뽑아줘", "TT_543_WIN 실행해줘", "윈도우에서 운영자 매뉴얼 만들어줘", "관리자 화면 캡쳐해서 PPT 만들어줘", "운영자 매뉴얼 산출물 만들어줘" 라고 말해도 이 스킬을 사용합니다. 단, PC 사용자 매뉴얼(입출고·재고 등 업무 화면)이 필요한 경우는 `/TT_541_WIN`, PDA 사용자 매뉴얼이 필요한 경우는 `/TT_542_WIN` 을 사용합니다.
type: skill
allowed-tools: Bash, PowerShell, Read, Write, Edit, Glob, Grep, AskUserQuestion
---

# 운영자 매뉴얼 PPTX 자동 생성 스킬 (Windows 전용) [TT_543_WIN]
...
## 실행 스크립트
1. `.claude/skills/TT_543_WIN/scripts/01_scan_admin_menus.js` (Node.js) — FE+BE 스캔 + 운영자 메뉴 필터
2. `.claude/skills/TT_543_WIN/scripts/02_capture_screens.js` (Node.js + Playwright chromium 헤드리스, ko-KR) — 메뉴별 캡처
3. `.claude/skills/TT_543_WIN/scripts/03_make_pptx.py` (Python + python-pptx + Pillow) — PPTX 생성
4. `.claude/skills/TT_543_WIN/scripts/parse_vue_source.py` (Python) — 우측 설명 패널을 채우기 위한 Vue 소스 파서
...
> **PPT 양식 (BLOCKING)**
> PPTX 의 레이아웃 · 색상 · 도형 라벨링 · 배지 · 커넥터 · 설명 패널 · 페이지 번호 규칙은 모두 **TT_541_WIN(PC 사용자매뉴얼) 스킬과 동일한 양식**을 따른다. 차이점은 (1) 표지 제목 "운영자 매뉴얼", (2) 대상 메뉴를 운영자/관리자 메뉴로만 필터링, (3) Windows 경로 사용 세 가지뿐이다.
>
> 템플릿 (`template/05 이행(TT)/사용자_매뉴얼_템플릿.pptx`) 는 TT_541_WIN 과 동일한 파일을 그대로 base 로 사용한다. 템플릿의 슬라이드 마스터 / 테마 / 레이아웃은 그대로 보존하고, 템플릿 안에 들어있던 예제 슬라이드는 모두 제거한 뒤 새 슬라이드를 추가한다.
> 템플릿이 없으면 스킬 실행을 중단하고 사용자에게 알린다.

> **PC/PDA 사용자 매뉴얼은 본 스킬의 범위가 아니다.** 입고·출고·재고 등 일반 업무 화면의 매뉴얼은 `/TT_541_WIN`(PC) 또는 `/TT_542_WIN`(PDA) 을 사용한다.
...
> 이 스킬은 PPT 생성 단계에서 TT_541_WIN 의 양식·도형·색상 상수를 그대로 차용한다. TT_541_WIN 스킬의 `scripts/03_make_pptx.py` 가 존재하면 동일 로직을 재사용하므로, 같은 저장소에서 TT_541_WIN / TT_542_WIN / TT_543_WIN 을 함께 유지하면 양식이 자동으로 동기화된다.
...
| **고객사명** | 출력 파일명 `TT_543_운영자매뉴얼_{고객사명}.pptx`의 `{고객사명}`. 윈도우 파일명 금지문자(`\ / : * ? " < > \|`)는 자동 `_` 치환 | `반다이남코` |
...
TEMPLATE   = template\05 이행(TT)\사용자_매뉴얼_템플릿.pptx     ← 필수 (TT_541_WIN과 동일 파일)
OUTPUT_DIR = output\05 이행(TT)
TMP_DIR    = output\05 이행(TT)\tmp_543
SCREEN_DIR = output\05 이행(TT)\tmp_543\screens\{메뉴코드}
SCRIPTS    = .claude\skills\TT_543_WIN\scripts
OUT_FILE   = output\05 이행(TT)\TT_543_운영자매뉴얼_{고객사명}.pptx
```

> **TT_541_WIN / TT_542_WIN 과 TMP 디렉토리 분리:** TT_541_WIN 은 `tmp_541`, TT_542_WIN 은 `tmp_542`, TT_543_WIN 은 `tmp_543` 를 사용하여 동시에 실행해도 충돌하지 않게 한다.
...
Set-Location "C:\zinide\workspace\cloud-wms-doc\.claude\skills\TT_543_WIN\scripts"
...
- 입고/출고/재고/반품/피킹/배송 관련 (`iw*`, `ob*`, `iv*`, `rt*`, `pk*`, `dl*`) — 일반 사용자 메뉴 (TT_541_WIN / TT_542_WIN 범위).
...
각 단계는 PowerShell 로 스크립트를 실행하고, 그 결과 JSON 을 다음 단계가 읽는 방식으로 진행된다. 각 단계 완료 후 산출물(`tmp_543\*.json` 또는 `screens\*.png`) 존재 여부를 확인한다.

```
.claude\skills\TT_543_WIN\scripts\
├── 01_scan_admin_menus.js   # 1단계 — FE + BE 스캔으로 운영자 메뉴 후보 추출
├── 02_capture_screens.js    # 2단계 — Playwright 헤드리스 캡처 (ko-KR, headless)
├── 03_make_pptx.py          # 3단계 — python-pptx 로 PPTX 생성 (TT_541_WIN 양식 그대로)
...
**출력**: `output\05 이행(TT)\tmp_543\admin_menu_candidates.json`

PowerShell:

```powershell
Set-Location "C:\zinide\workspace\cloud-wms-doc"
node ".claude\skills\TT_543_WIN\scripts\01_scan_admin_menus.js" "{FE경로}" "{BE경로}"
```

스크립트가 수행하는 일:

1. **FE 측 스캔** (TT_541_WIN 의 `01_scan_project.js` 와 동일 로직 + 운영자 필터):
...
확정된 값은 `output\05 이행(TT)\tmp_543\capture_config.json` 으로 저장한다.
...
**입력**: `tmp_543\capture_config.json`
**출력**:
- `tmp_543\screens\{메뉴코드}\01-main.png`
- `tmp_543\screens\{메뉴코드}\02-search-result.png`
- `tmp_543\screens\{메뉴코드}\03-register-popup.png` (등록 시나리오 있는 경우)
- `tmp_543\screens\{메뉴코드}\04-row-selected.png`
- `tmp_543\screens\{메뉴코드}\05-edit-popup.png` (수정 시나리오 있는 경우)
- `tmp_543\screens\{메뉴코드}\coords.json` (각 영역 DOM bounding box)
- `tmp_543\screens\{메뉴코드}\buttons\*.png` (툴바 / 팝업 버튼 아이콘)

PowerShell:

```powershell
Set-Location "C:\zinide\workspace\cloud-wms-doc"
node ".claude\skills\TT_543_WIN\scripts\02_capture_screens.js"
```
...
### 4단계 — PPTX 생성 (TT_541_WIN 양식 그대로, 표지·파일명만 운영자 매뉴얼로)

**스크립트**: `scripts\03_make_pptx.py` (python-pptx)

**입력**:
- 템플릿: `template\05 이행(TT)\사용자_매뉴얼_템플릿.pptx` (필수, TT_541_WIN 과 동일 파일)
- `tmp_543\capture_config.json`
- `tmp_543\screens\{메뉴코드}\*.png`
- `tmp_543\screens\{메뉴코드}\coords.json`
...
```powershell
Set-Location "C:\zinide\workspace\cloud-wms-doc"
python ".claude\skills\TT_543_WIN\scripts\03_make_pptx.py"
```

#### TT_541_WIN 와 동일한 처리 (BLOCKING)

다음 항목은 TT_541_WIN 의 `03_make_pptx.py` 와 **완전히 동일하게** 처리한다. 양식 변경은 금지한다.
...
6. **설명 패널 톤**: TT_541_WIN 은 "사용자가 일상 업무에서 사용"하는 톤이지만, TT_543_WIN 은 "운영자가 시스템을 설정"하는 톤. ui.md 가 있으면 그대로 사용하되, 자동 생성되는 디폴트 문구는 운영자 톤(예: "신규 사용자 등록 시 [추가] 버튼을 클릭합니다", "권한 변경 후 [저장] 클릭")으로 살짝 다르게 처리.
...
PPTX 생성 성공 후, 비밀번호가 포함된 `capture_config.json` 을 보호하기 위해 `tmp_543\` 폴더를 삭제할지 사용자에게 묻는다.

```powershell
# 사용자가 'Yes' 선택 시
Remove-Item -Recurse -Force "C:\zinide\workspace\cloud-wms-doc\output\05 이행(TT)\tmp_543"
```

- 실패한 경우(연결 실패·캡처 실패 등)에는 디버깅을 위해 `tmp_543\` 를 그대로 두고, 사용자에게 원인을 보고한다.
...
PPT 양식은 TT_541_WIN 사용자매뉴얼과 동일하며, 표지 제목만 "운영자 매뉴얼"로 다릅니다.
```
...
2단계에서 해당 메뉴만 선택하면 된다. `tmp_543\screens\{메뉴코드}\` 는 메뉴별로 분리되므로
다른 메뉴의 캡처는 보존된다.

PPTX 는 매번 `OUT_FILE` 경로에 **전체 다시** 작성된다 (부분 슬라이드 패치는 지원하지 않음).
이전 산출물을 보존해야 하면 실행 전에 파일을 백업하거나, `tmp_543\capture_config.json` 의 `menus[]` 에 모든 메뉴를 명시한다.
...
| PPT 양식이 TT_541_WIN 과 미세하게 다름 | `03_make_pptx.py` 가 별도로 유지되어 동기화 안 됨 | 본 스킬의 `03_make_pptx.py` 는 TT_541_WIN 의 로직을 그대로 복제. 양식 변경이 필요하면 TT_541_WIN / TT_542_WIN / TT_543_WIN 양쪽을 함께 수정한다. |
...
- **비밀번호 노출 방지:** 5단계에서 `tmp_543\capture_config.json` 안에 평문으로 저장된 비밀번호를 보호하기 위해 폴더 삭제 여부를 묻는다. 비정상 종료 시 폴더가 남아 있으면 즉시 수동 삭제한다.
- **함께 보면 좋은 스킬:**
  - PC 사용자 매뉴얼 PPTX → `/TT_541_WIN`
  - PDA 사용자 매뉴얼 PPTX → `/TT_542_WIN`
  - 프로그램 목록 엑셀 → `/PI_412`
