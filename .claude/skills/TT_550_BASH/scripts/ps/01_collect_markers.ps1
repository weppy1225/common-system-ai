# TT_550 3단계 (POWERSHELL 모드) - psql 로 @migrate: 마커 + FK 의존성 수집
#
# 사용법:
...
"@
    ...
    if ($null -eq $order) {
        ...
        Write-Host "[TT_550] ⚠ $gk : FK 순환 의존성 감지" -ForegroundColor Yellow
    } else {
...
Write-Host "[TT_550] 마커 스캔 완료: $($groups.Count) 그룹 / $($totalTables.Sum) 테이블 → $OutFile" -ForegroundColor Green
...
if ($unmarked.Count -gt 0) {
    Write-Host "[TT_550] ⚠ 마커 없는 sm_/mdm_ 테이블 $($unmarked.Count)개: $($unmarked -join ', ')" -ForegroundColor Yellow
}
