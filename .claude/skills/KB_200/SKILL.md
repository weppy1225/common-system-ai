...
프로젝트명({프로젝트})을 워크스페이스 폴더명에서 도출(2단계 경로 계산 규칙, → `.claude/rules/repo-paths.md`)한 뒤 `spec/{프로젝트}/_knowledge/menu-list.md` 를 Read 한다.
...
KB 문서 폴더(`spec/{프로젝트}/{MENU_CODE}/`)가 없거나 `.md` 파일이 0개이면:
