---
name: TT_550_WIN
description: 【DB 이관용 SQL 준비물 생성 (실DB 접속, Windows 전용, Python/PS 폴백)】 Windows 네이티브(PowerShell) 환경에서 사용자가 지정한 백엔드 디렉토리의 DB 설정 파일을 자동 스캔해 인하우스 PostgreSQL DB에 접속하고, `COMMENT ON TABLE` 의 `@migrate:` 마커가 달린 테이블만 자동 수집하여 그룹별 INSERT SQL 파일과 메타데이터(manifest.json)를 생성합니다. 산출물은 `output/05 이행(TT)/TT_550_DATA_{고객사명}_{YYMMDD}/` 폴더에 그룹별 `.sql` 파일 + `manifest.json` 으로 떨어지며, 이 폴더는 후속 스킬 /TT_551_WIN 의 입력이 됩니다. SQL 파일은 도구 중립적 plain SQL(Flyway 비의존)이라 고객사는 `psql -f` 한 줄로 적용 가능합니다. /TT_550_WIN 형식으로 실행하며 백엔드 경로·고객사명·dump 그룹은 실행 시 묻습니다. 실행 모드는 자동 감지: Python+psycopg2 가 있으면 PYTHON 모드(psycopg2로 wire protocol 접속, pg_dump 버전 충돌 무관), 없으면 POWERSHELL 모드(psql/pg_dump --inserts), 둘 다 없으면 에러로 종료합니다. 핵심 설계: (1) AI는 DB에 직접 INSERT/UPDATE 를 쏘지 않고 결정론적 SQL 텍스트만 생성, (2) 매핑은 DB 자체의 COMMENT 에 박혀 있어 외부 YAML/상수 동기화 불필요, (3) 고객사 대상 DB 적용은 옵션으로 마지막에 묻기(기본 No, Yes 시 host 재확인). DB 이관용 SQL 준비물 생성, dump SQL 파일 만들기, 공통코드/마스터 데이터 INSERT 스크립트 만들기, 이관계획서 생성 전 SQL 패키지 만들기 요청 시 반드시 이 스킬을 사용합니다. 사용자가 "DB 이관 SQL 만들어줘", "공통코드 데이터 dump 떠줘", "마스터 데이터 INSERT 스크립트", "TT_550_WIN 실행해줘", "윈도우에서 DB 데이터 dump", "이관계획서용 SQL 패키지" 라고 말해도 이 스킬을 사용합니다. 단, DDL(스키마) 만 필요하면 /SD_333_WIN, DB 이관계획서 엑셀이 필요하면 /TT_551_WIN(이 스킬 실행 후 호출)을 사용합니다. Linux/WSL/macOS 환경에서는 별도의 기본 TT_550 스킬을 사용합니다(있는 경우).
allowed-tools: Bash, Read, Write, Edit, AskUserQuestion
---

# DB 이관용 SQL 준비물 생성 (실 DB 접속, Windows 전용, Python/PS 폴백) [TT_550_WIN]
...
>
> **이 스킬은 후속 `/TT_551_WIN` (이관계획서 엑셀 생성) 의 입력을 만든다.**
> 흐름: `TT_550_WIN` (SQL 준비물 + manifest) → `TT_551_WIN` (이관계획서 엑셀)
>
> **설계 원칙 3가지:**
...
TMP_DIR    = output\05 이행(TT)\TT_550_DATA_{고객사명}_{YYMMDD}\tmp
SCRIPTS    = .claude\skills\TT_550_WIN\scripts
```
...
```
.claude\skills\TT_550_WIN\scripts\
...
elseif ($pgOk) { $mode = "POWERSHELL" }
    ...
    Write-Host "ERROR: TT_550_WIN 실행을 위해 다음 중 하나가 필요합니다:" -ForegroundColor Red
...
Write-Host "[TT_550_WIN] 실행 모드: $mode" -ForegroundColor Cyan
```
...
호출:
```powershell
python ".claude\skills\TT_550_WIN\scripts\_scan_config.py" "{BE경로}" "{출력 json 경로}"
```

아래 패턴 파일을 재귀로 찾아 후보를 모은다 (SD_333_WIN 의 검증된 로직 재사용).
...
```sql
-- =============================================================
-- TT_550_WIN: 01_common_code (공통코드)
-- 고객사:     {고객사명}
...
```powershell
Remove-Item -Recurse -Force "C:\zinide\workspace\cloud-wms-doc\output\05 이행(TT)\TT_550_DATA_{고객사명}_{YYMMDD}\tmp"
```
...
```
✓ DB 이관 SQL 준비물 생성 완료 [TT_550_WIN]

실행 모드:    PYTHON (psycopg2 3.1.18)
백엔드 경로:  C:\zinide\workspace\cloud-wms-be
고객사:       {고객사명}
...
다음 단계:
  → /TT_551_WIN 실행 시 위 출력 폴더를 입력으로 받아 DB 이관계획서 엑셀을 생성합니다.
...
- **Windows·WSL 경로 자동 변환:** `_scan_config.ps1` 가 `/mnt/c/...` ↔ `C:\...` 자동 정규화.
...
- **함께 보면 좋은 스킬:**
  - DB 이관계획서 엑셀 → `/TT_551_WIN` (이 스킬 실행 후 호출)
