...
foreach ($g in $targetGroups) {
    ...
    if ($null -eq $g.insert_order -or $insertOrder.Count -eq 0) {
        Write-Host "[TT_550_WIN] ⚠ ${gk}: FK 순환 의존성으로 skip" -ForegroundColor Yellow
        continue
    }
    ...
    $headerLines = @()
    $headerLines += "-- ============================================================="
    $headerLines += "-- TT_550_WIN: $gk ($gdesc)"
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "[TT_550_WIN] ⚠ ${gk}: pg_dump 실패 (exit=$LASTEXITCODE)" -ForegroundColor Yellow
    ...
    Write-Host "[TT_550_WIN] ✓ [$gk] $gdesc : $totalRows rows → $gk.sql ($sizeKb KB)" -ForegroundColor Green
}
