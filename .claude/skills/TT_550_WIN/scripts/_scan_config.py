#!/usr/bin/env python3
"""
TT_550_WIN 1단계 - 디렉토리 스캔으로 PostgreSQL 접속정보 후보 추출.
...
TT_550_WIN 은 출력 경로가 고객사명·날짜에 따라 달라지므로 두 번째 인자로 받는다.
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fn in filenames:
            ...
            try:
                ...
                print(f"[TT_550_WIN] WARN: 파싱 실패 {rel}: {e}", file=sys.stderr)
                continue
            if new_cands:
...
def main():
    ...
    if not target.exists() or not target.is_dir():
        print(f"[TT_550_WIN] 디렉토리가 존재하지 않습니다: {target}", file=sys.stderr)
        sys.exit(2)
    ...
    print(f"[TT_550_WIN] 후보 {len(cands)} 건 → {out_path}")
