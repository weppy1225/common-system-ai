#!/usr/bin/env python3
"""PI_421_WIN 1단계 — JUnit 테스트 메서드 스캔 (Windows 전용).
...
PI_421 원본과 동일하나 다음 두 가지만 다르다:
1. BASE_DIR을 `Path(__file__).resolve().parents[4]` 로 자동 추론 (Windows/WSL 무관).
2. normalize_path()가 WSL 경로(`/mnt/c/...`) → Windows 경로(`C:\\...`) 로 변환.
"""
...
# .claude/skills/PI_421_WIN/scripts/01_scan_tests.py
# parents[0]=scripts  parents[1]=PI_421_WIN  parents[2]=skills
