...
AskUserQuestion으로 BASE_URL / dev 서버 / 관리자 메뉴(multiSelect) / 관리자 로그인 정보 / 고객사명 / 시스템명 / 뷰포트 확정.
...
1. **표지** → 제목 "관리자매뉴얼", 부제 "{고객사명} {시스템명}", 작성일자
...
> 단, `spec/{프로젝트}/{메뉴코드}/{메뉴코드}-02-ui.md` 는 이 스킬에서 사용하지 않는다. Vue 소스가 더 정확.
