---
name: md_index
description: 레포 파일·폴더 지도(knowledgebase/20-md-index.html)를 scripts/gen-md-map.py 로 재생성해 최신화한다. /md_index
when_to_use: "파일 지도 최신화", "md-index 갱신", "20-md-index.html 다시 생성", "문서 지도 업데이트", "md_index 실행" 요청 시 사용.
allowed-tools: Bash
---

# 파일 지도 최신화 [md_index]

레포의 파일·폴더 지도 `knowledgebase/20-md-index.html` 를 **생성 스크립트로 재생성**한다.
...
- 스크립트는 CWD와 무관하게 레포 루트 기준으로 동작하며, 출력은 `knowledgebase/20-md-index.html`.

## 2단계 — 결과 확인

```bash
cd "$(git rev-parse --show-toplevel)"
git status -s knowledgebase/20-md-index.html
ls -l knowledgebase/20-md-index.html
```
