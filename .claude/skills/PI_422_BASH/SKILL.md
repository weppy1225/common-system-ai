---
name: PI_422_BASH
description: 【통합테스트보고서 엑셀 생성 (WSL/Linux/Mac)】 dist/ 폴더의 ui.md 파일을 스캔하여 메뉴별 통합테스트 시나리오를 자동 생성하고, template/04 구현(PI)/PI.214-통합테스트보고서.xlsx 템플릿에 채워 output/04 구현(PI)/PI.422_통합테스트보고서_{고객사명}.xlsx 로 저장합니다. /PI_422_BASH 형식으로 실행하며 고객사명·담당자·테스트 기간은 실행 시 묻습니다. WSL/Linux/Mac 환경에서 통합테스트 보고서 작성, 테스트 시나리오 정리, 통합테스트 결과 엑셀 만들기 요청 시 반드시 이 스킬을 사용합니다. Windows 환경에서는 PI_422 스킬을 사용합니다.
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, AskUserQuestion
---

# 통합테스트보고서 자동 생성 [PI_422_BASH]
...
OUTFILE="$OUTPUT_DIR/PI.422_통합테스트보고서_{고객사명}.xlsx"
...
> **스크립트 공유**: `gen_pi422.js`는 PI_422와 PI_422_BASH가 공유한다. 스크립트 내부에서 `path.resolve(__dirname, '..', '..', '..', '..')` 패턴으로 BASE_DIR을 동적 감지한다.
...
- **스크립트 공유**: `gen_pi422.js`는 PI_422 디렉토리의 것을 사용한다. PI_422_BASH 에는 별도 scripts 폴더가 없다.
...
- **WSL/Linux/Mac 전용**: Windows 환경에서는 PI_422 스킬을 사용한다.
