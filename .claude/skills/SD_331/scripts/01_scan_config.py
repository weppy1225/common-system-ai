#!/usr/bin/env python3
"""
SD_331 1단계 - 디렉토리 스캔으로 DB 접속정보 후보 추출.
...
def main():
    ...
    print(f"[SD_331] 디렉토리 스캔 시작: {target}")
    found = scan_directory(target)
    print(f"[SD_331] 후보 발견: {len(found)}개 (병합 전)")

    cands = dedupe(found)
    print(f"[SD_331] 후보 병합 후: {len(cands)}개")
