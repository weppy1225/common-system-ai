...
def load_target():
    if not TARGET_FILE.exists():
        print(f"[SD_331] 접속정보 파일 없음: {TARGET_FILE}", file=sys.stderr)
        sys.exit(2)
...
def main():
    ...
    if driver not in EXTRACTORS:
        print(f"[SD_331] 지원하지 않는 driver: {driver}", file=sys.stderr)
        sys.exit(2)

    missing = check_libs(driver)
    if missing:
        print(f"[SD_331] 라이브러리 누락:", file=sys.stderr)
        for m in missing:
    ...
    print(f"[SD_331] DB 연결 시도: {driver} {target.get('host')}:{target.get('port')}/{target.get('database')}")
    try:
        conn = connect(target)
    except Exception as e:
        print(f"[SD_331] 연결 실패: {e}", file=sys.stderr)
        sys.exit(3)
    ...
    try:
        ...
        print(f"[SD_331] 스키마 추출 중 오류: {e}", file=sys.stderr)
        raise
    finally:
        try:
            conn.close()
    ...
    print(f"[SD_331] 추출 완료: {SCHEMA_FILE}")
