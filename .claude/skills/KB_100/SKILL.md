---
name: KB_100
description: 【지식베이스 자동 생성】 메뉴코드를 입력받아 BE(Java) + FE(Vue) 소스를 분석하고 spec/{프로젝트}/{메뉴코드}/ 에 01~07 + 99 총 8개 문서를 자동 생성한다. /KB_100 {메뉴코드} 형식으로 실행. 기존 문서가 있으면 bakup{YYMMDD}{순번} 폴더로 백업 후 새로 생성한다. 사용자가 "지식베이스 만들어줘", "KB 문서 생성", "소스 분석 문서 뽑아줘", "KB_100 실행해줘" 라고 말해도 이 스킬을 사용한다.
...
프로젝트명({프로젝트})을 워크스페이스 폴더명에서 도출(2단계 `$Prefix` 규칙, → `.claude/rules/repo-paths.md`)한 뒤 `spec/{프로젝트}/_knowledge/menu-list.md` 를 Read 한다.
...
서브에이전트 완료 후 `spec/{프로젝트}/_knowledge/menu-list.md` 의 해당 행 상태를 `완료`로 갱신한다.

```powershell
$menuListPath = "spec\$Prefix\_knowledge\menu-list.md"
