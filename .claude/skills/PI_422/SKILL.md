...
`spec/` 아래 모든 `ui.md` 파일을 스캔하여 메뉴 목록을 추출하고,
...
### 2-1. spec/ 스캔 + prototype/_common-m/menu.html 스캔

- `spec/` 하위 모든 폴더에서 `ui.md` 파일을 탐색한다 (WEB 메뉴).
- `prototype/_common-m/menu.html` 에서 `.menu-cell` href 를 파싱하여 PDA 메뉴 목록 추출.
  - 메뉴코드: href 경로에서 추출 (예: `iv3000m/IVMV01.html` → `IVMV01`)
  - 메뉴명: `<img alt="...">` 값 사용
- `spec/{menuCode.toLowerCase()}/{menuCode.toLowerCase()}-02-ui.md` 있으면 해당 파일에서 (메뉴그룹명·메뉴명·목적) 사용
...
- **spec/ 없으면**: 스크립트가 경고 메시지를 출력하고 종료한다.
