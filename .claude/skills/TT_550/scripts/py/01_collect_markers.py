#!/usr/bin/env python3
"""
TT_550 3단계 (PYTHON 모드) - @migrate: 마커 + FK 의존성 수집.
...
def main():
    ...
    for gk, g in groups.items():
        ...
        if order is None:
            g["insert_order"] = None
            g["delete_order"] = None
            print(f"[TT_550] ⚠ {gk}: FK 순환 의존성 감지 — 적용 순서를 수동 결정 필요", file=sys.stderr)
        else:
    ...
    print(f"[TT_550] 마커 스캔 완료: {len(groups)} 그룹 / {total_tables} 테이블 → {out_file}")
    ...
    if unmarked:
        print(f"[TT_550] ⚠ 마커 없는 sm_/mdm_ 테이블 {len(unmarked)}개 (혹시 누락?): {', '.join(unmarked[:5])}{' ...' if len(unmarked) > 5 else ''}")
