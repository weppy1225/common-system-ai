---
name: PI_421_BASH
description: 【단위테스트 보고서 엑셀 생성 (WSL/Linux/Mac)】 WSL·Linux·macOS(Bash) 환경에서 사용자가 지정한 로컬 백엔드(Java/Kotlin) 디렉토리를 자동 스캔하여 모든 JUnit 테스트 코드(@Test 메서드)를 추출하고, 모든 테스트가 통과(결과=O)되었다는 가정 하에 단위테스트보고서 엑셀을 자동 생성합니다. 템플릿은 `template/04 구현(PI)/PI_212-단위테스트보고서.xlsx` 를 그대로 복사해 사용하며, `output/04 구현(PI)/PI_421_단위테스트보고서_{고객사명}_{YYMMDD}.xlsx` 로 저장합니다. /PI_421_BASH 형식으로 실행하며 디렉토리·고객사명·담당자명은 실행 시 묻습니다. JUnit 4(`org.junit.Test`) 와 JUnit 5(`org.junit.jupiter.api.Test`) 양쪽을 모두 인식하고, `@DisplayName` 이 있으면 우선 사용합니다. 주석 처리된(`//@Test`) 테스트는 제외합니다. 단위테스트 보고서 작성, JUnit 테스트 목록 정리, 백엔드 단위테스트 산출물 만들기 요청 시 반드시 이 스킬을 사용합니다. 사용자가 "단위테스트보고서 만들어줘", "JUnit 테스트 정리해줘", "단위테스트 산출물 뽑아줘", "PI_421_BASH 실행해줘", "BE 단위테스트 엑셀" 이라고 말해도 이 스킬을 사용합니다. 단, 통합테스트(PI_214) 산출물이 필요한 경우에는 별도 스킬을 사용합니다. Windows 환경에서는 기본 PI_421 스킬을 사용합니다.
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, AskUserQuestion
---

# 단위테스트 보고서 자동 생성 (WSL/Linux/Mac) [PI_421_BASH]

지정된 백엔드(Java/Kotlin) 디렉토리의 **JUnit 테스트 코드 전수(全數)**를 스캔하여,
모든 테스트가 **통과(결과=O)** 되었다는 가정 하에 단위테스트 보고서를 엑셀로 생성한다.

> **실행 환경:** WSL · Linux · macOS(Bash/zsh). `python3` 명령이 필요하다.
>
...
| 입력 | 설명 |
|---|---|
| 백엔드 디렉토리 경로 | 스캔할 Java/Kotlin 프로젝트 루트의 절대경로. (기본 제안: `$BE_ROOT`) |
...
```
BASE       = $DOC_ROOT  (git rev-parse --show-toplevel로 자동 감지)
...
각 단계는 Bash로 `.claude/skills/PI_421_BASH/scripts/` 안의 Python 스크립트를 실행한다.
산출물 JSON이 정상 생성됐는지 확인한 뒤 다음 단계로 넘어간다.
중간 단계에서 실패하면 `tmp/` 를 남겨 디버깅한다.

```
.claude/skills/PI_421_BASH/scripts/
...
DOC_ROOT=$(git rev-parse --show-toplevel)
cd "$DOC_ROOT" && \
python3 -u .claude/skills/PI_421_BASH/scripts/01_scan_tests.py "{디렉토리경로}"
```
...
DOC_ROOT=$(git rev-parse --show-toplevel)
cd "$DOC_ROOT" && \
python3 -u .claude/skills/PI_421_BASH/scripts/02_generate_excel.py "{고객사명}" "{담당자명}"
```
...
```bash
DOC_ROOT=$(git rev-parse --show-toplevel)
cd "$DOC_ROOT" && rm -rf "output/04 구현(PI)/tmp"
```
