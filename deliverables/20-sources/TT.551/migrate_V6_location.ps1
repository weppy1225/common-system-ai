...
# 위치(로케이션) 이관: test → dev  (psql \copy 방식)
# 대상: mdm_loc
# 실행: .\migrate_V6_location.ps1
# ============================================================

. "$PSScriptRoot\db_config.ps1"

$SCRIPT_NAME = "V6__location"
$TSV_DIR     = $env:TEMP
...
[System.IO.File]::WriteAllText($tmpDel, "DELETE FROM mdm_loc;", $utf8NoBom)
...
$verifySql = "SELECT 'mdm_loc' AS tbl, COUNT(*) AS cnt FROM mdm_loc;"
