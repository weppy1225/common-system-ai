...
related:
  - 10-src-pattern/40-frontend/10-architecture/02-be-fe-contract.md
  - 10-src-pattern/30-backend/90-api/
---

# BE 스펙 소비(Consume) 컨벤션

FE AI 가 BE(`cloud-wms-be`) spec 산출물을 직접 읽고, 코드와 메뉴 문서에 반영할 때 따르는 **단일 소스**.
BE-FE 런타임 계약(HTTP 메서드·응답 네이밍·복합키 등)은 `10-src-pattern/40-frontend/10-architecture/02-be-fe-contract.md` 에 있고, 이 문서는 그 계약을 **어떤 문서에서 어떻게 끌어오는가** 를 정의한다.

## 1. BE 스펙 원천(source of truth)

BE 저장소: `cloud-wms-be/DEV_DOC/ai-docs/`
...
| FE 소비용 API 상세 | `10-src-pattern/30-backend/90-api/20-detail/{menu-lower}-{method}-{res}.md` |
| 도메인별 API 요약 | `10-src-pattern/30-backend/90-api/10-domain/{domain}-api.md` |
| 전체 API 목록 | `10-src-pattern/30-backend/90-api/00-backend-api-overview.md` |
| 테이블 명세 | `10-src-pattern/30-backend/` (DB 스키마) |
...
4. `10-src-pattern/30-backend/90-api/20-detail/*.md` — API 단위 상세. 80-spec 만으로 부족할 때 보조 참고.
...
- API 목록, Request/Response, 복합키, 공통코드는 매번 `cloud-wms-be/.../80-spec/{menu}/` 에서 직접 확인한다.
- FE 메뉴 문서(`30-domain/{업무군}/{메뉴코드}/`)에는 FE 소비에 필요한 매핑과 최근 동기화 날짜만 기록한다.
...
1. `cloud-wms-be/DEV_DOC/ai-docs/20-backend/80-spec/{menu-lower}/` 존재 여부 확인.
2. 최신 `{YYYYMMDD}_output.md`, 없으면 `output.md`, 없으면 `spec.md` 를 읽는다.
3. 필요하면 `10-src-pattern/30-backend/90-api/20-detail/{menu-lower}-*.md` 를 보조 참고한다.
4. 작업 완료 후 `30-domain/{업무군}/{메뉴코드}/` 의 API 매핑표를 80-spec 기준으로 갱신한다.
