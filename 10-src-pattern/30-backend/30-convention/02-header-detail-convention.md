...
> 단순 마스터(`mdpd01`)에서는 `POST /insert`, `POST /update` 사용 — 혼용 금지
...
public {메뉴코드}Response update{헤더리소스}({메뉴코드}{헤더리소스} patch{헤더리소스}) {
    ...
    finally { result.setProcCnt(retCnt); }
    return result;
}
...
WMSPool.STAND_BY // "11" - 대기 상태 (수정/삭제 가능)
WMSPool.CONFIRMED // "20" - 확정 상태 (수정/삭제 불가)
WMSPool.COMPLETED // "30" - 완료 상태
```
...
DocNoBean bean = DocNoBean.docNoPubBuilder()
        ...
        .build();
docNoGenerator.getDocNo(bean);
String docNo = bean.getDocNo(); // "IW20260101001"
