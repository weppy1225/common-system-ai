...
>
> 실제 BE 소스 기준 공통 예외 예시: `ZinRequestParamValidException`, `ZinExistDataException`, `ZinNotFoundException`, `NotMeetConditionsException`, `AlreadyProcessException`, `ResponseErrorException`.
...
## 17. 금지 패턴 (SSoT 위임, MUST/NEVER)

> MUST: 백엔드 레이어 금지 패턴은 `.claude/rules/backend-convention.md §7`를 기준으로 판단한다.
> NEVER: 중복 정의로 재서술하지 말고 DB·쿼리 금지 패턴은 `.claude/rules/db-convention.md §6`에 위임한다.
