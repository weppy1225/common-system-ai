...
tags:
  ...
  - annotation
relates_to:
  - 10-src-pattern/30-backend/40-guide/02-controller-writing-rules.md
  ...
  - 10-src-pattern/30-backend/40-guide/07-computil-writing-rules.md
...
> **판단 기준 (SSoT)**: `.claude/rules/backend-convention.md §3` 참조 — Comp·TxComp 분리 표, `@Transactional` 선언 위치 규칙 포함.
...
- Mapper: 인터페이스. `@Param` 사용 기준 → `.claude/rules/db-convention.md §2` 참조.
...
| DTO 종류 | 상위 클래스 | 사용처 |
|---|---|---|
| `{메뉴코드}Response` | `extends ResponseData` | 모든 API 응답 (목록·단건·처리건수 포함) |
...
**이력 컬럼**: `regId`, `regDt`, `modId`, `modDt` 필드는 자동 세팅 대상이며 입력값으로 받지 않음.
...
> 예외 클래스 목록 및 선택 기준 (SSoT): `.claude/rules/backend-convention.md §5` 참조.
>
> 이 문서 고유 참조: `CompWarnException` (비즈니스 경고), `ResponseWarnException` / `ResponseErrorException` (Comp → Controller 전달 전용).

### 9.2 예외 처리 원칙

- `RuntimeException` 직접 throw 금지 — 위 목록에서 가장 가까운 예외 사용.
- Comp 메서드의 try 블록은 5줄 이내. 복잡한 검증·세팅 로직은 CompUtil로 분리.
...
| `MsgTool.getMessage("key")` / `getMsgParam("key", ...)` | 다국어 메시지 조회 |
...
> 백엔드 레이어 금지 패턴 (SSoT): `.claude/rules/backend-convention.md §10` 참조.
> DB·쿼리 금지 패턴 (SSoT): `.claude/rules/db-convention.md §9` 참조.
