...
WMS AI 프레임워크 허브 레포의 **목표 최상위 구조**를 고정한다.

> ✅ **2026-06-17 적용 완료 (Phase A~F).** 현재 구조는 `STRUCTURE.md`에 반영됐다. 이 문서는 **재설계 결정 근거·이전 매핑 기록**으로 보존한다. (단, 모바일 평탄화 `{그룹}m→{메뉴}m`, KB_100/KB_200 재설계는 별도 후속 작업으로 남음)
