---
title: 공통 UI 패턴 (WEB)
description: 모든 와이어프레임 HTML에 공통 적용되는 페이지 래퍼·헤더·모달·그리드·버튼·공통 팝업(거래처/품목)·테스트 데이터·테스트 항목의 상세 구현 패턴. 얇은 rule(common_ui.md)이 이 문서를 SSoT로 참조한다.
status: active
version: 1.0.0
author: ShinHyunKyu
repo_role: ai-hub
...
related:
  ...
  - .claude/rules/common_ui.md
---

# 공통 UI 패턴 (WEB)

> 이 문서는 와이어프레임 HTML 공통 영역의 **상세 구현 SSoT**다. 판단 기준(금지/필수)은 얇은 rule `.claude/rules/common_ui.md`에 있으며, 구현 시 이 문서를 연다.
...
업무규칙 팝업의 화면구성 섹션에 아래 항목을 반드시 기입한다.
...
### 공통
- CRUD 버튼(추가/수정/삭제/복사)은 아이콘 버튼(`btn-icon`)으로 표시한다. 텍스트 없이 SVG 아이콘만 사용한다. (상세 규칙: `03-toolbar-buttons.md`)
...
- 업무규칙 팝업 상세 → `07-popup-biz-rule.md`.
...
- 그리드 하위에 페이징 컨트롤(페이지 번호, 페이지 크기 셀렉터, 건수 표시)을 추가한다. (상세 규칙: `04-result-grid.md`)
...
- **아이콘 버튼은 반드시 `prototype/{프로젝트}/_common/icon-preview.html`에 정의된 아이콘만 사용한다.** icon-preview.html에 없는 아이콘은 임의로 만들거나 추가하지 않으며, 해당하는 아이콘이 없는 기능 버튼은 텍스트 버튼으로 처리한다.
...
- 화면설계 패턴 전체 인덱스: → `patterns/10-screen-design/00-overview.md`
