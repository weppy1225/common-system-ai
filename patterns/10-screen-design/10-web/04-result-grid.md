---
title: 결과 그리드 영역 패턴 (WEB)
description: 검색 결과 데이터 그리드(Data Grid) 영역의 헤더·셀·컬럼 너비 기준표·정렬·체크박스·가로 스크롤·페이징·다중 패널 높이 제어 상세 구현 패턴. 얇은 rule(area_result_grid.md)이 이 문서를 SSoT로 참조한다.
status: active
version: 1.0.0
author: ShinHyunKyu
repo_role: ai-hub
...
related:
  ...
  - .claude/rules/area_result_grid.md
---

# 결과 그리드 영역 패턴 (WEB)

> 결과 그리드의 **상세 구현 SSoT**다. 판단 기준(금지/필수)과 컬럼 너비 BLOCKING 요약은 얇은 rule `.claude/rules/area_result_grid.md`에 있으며, 컬럼 너비 기준표 전체와 CSS 상세는 이 문서가 정본이다.
