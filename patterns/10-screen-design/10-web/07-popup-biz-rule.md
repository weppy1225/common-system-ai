---
title: 업무규칙 팝업 패턴 (WEB)
description: 업무규칙 버튼 클릭 시 표시되는 모달 팝업의 화면구성 테이블(4행 고정)·업무규칙 목록·드래그 이동·HTML/CSS 참조 패턴. 얇은 rule(popup_biz.md)이 이 문서를 SSoT로 참조한다.
status: active
version: 1.0.0
author: ShinHyunKyu
repo_role: ai-hub
...
related:
  ...
  - .claude/rules/popup_biz.md
---

# 업무규칙 팝업 패턴 (WEB)

> 업무규칙 팝업의 **상세 구현 SSoT**다. 판단 기준(금지/필수)은 얇은 rule `.claude/rules/popup_biz.md`에 있다.
