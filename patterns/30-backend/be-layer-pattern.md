...
모든 메뉴에 동일하게 적용되는 F/W 기반 레이어 구조다. 메뉴별 `06-be-flow.md`에는 이 구조를 반복하지 않고 업무별 시퀀스만 기술한다.
...
| 레이어 | 명명 규칙 | 역할 |
|---|---|---|
| Controller | `{메뉴코드}Controller` | HTTP 요청 수신·경로 매핑, 토큰에서 로그인 사용자 정보 추출 후 Comp에 위임. 업무 로직 없음 |
| Comp | `{메뉴코드}Comp` | 비즈니스 규칙 검증(필수값·존재 여부), 예외 변환, TxComp 또는 Dao 호출 조합. `@Transactional` 없음 |
...
| CompUtil | `{메뉴코드}CompUtil` | Comp 공통 유틸. 있는 메뉴에만 존재 |
| bean | `bean/*.java` | 요청·응답·내부 전달 VO |
| Mapper.xml | `{메뉴코드}Mapper.xml` | SQL 정의 (MyBatis) |
...
- 예외는 F/W 공통 예외 클래스(§3)로 throw 하고, Controller가 에러 응답으로 변환한다.
